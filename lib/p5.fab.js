...
p5.prototype.createFab = function () {
  // Pass the current p5 instance to Fab for safe fallback when WEBGL is not available
  _fab = new Fab(undefined, this);
  return _fab;
};
...
p5.prototype.saveShape = function () {
  if (this._renderer.isP3D) {
    ...
    Debug.warn("Don't use saveShape in 2D mode.");
  }
};
...
class LinearMove {
  constructor(cmdString) {
    ...
    const commentIdx = cmdString.indexOf(';'); // Note: Custom tags for MIDI control could be added here in the future
    if (commentIdx > -1) {
...
class Fab {
  constructor(config = defaultPrinterSettings, p5Instance = null) {
    ...
    // Camera for viz (provides safe fallback when WEBGL is not available)
    const createSafeCamera = () => {
      return {
        // Keep interface consistent with p5.Camera
        eyeX: 0,
        eyeY: 0,
        eyeZ: 0,
        centerX: 0,
    ...
    let cameraObj = null;
    // Prefer instance method (valid when p5 instance exists and WEBGL is enabled)
    if (p5Instance && typeof p5Instance.createCamera === 'function') {
      try {
        cameraObj = p5Instance.createCamera();
      } catch (e) {
        cameraObj = null;
      }
    }
    // Then try global method (available in global WEBGL sketches)
    if (!cameraObj && typeof createCamera === 'function') {
      try {
    ...
    // Finally use fallback stub object to avoid crash when using serial-only functionality
    if (!cameraObj) {
      cameraObj = createSafeCamera();
    }
  ...
  fabscription(commandToSend) {
    ...
    if (this.autoHomeComplete && !this.bufferFilled) {
      // Standard G-code commands that are buffered by most firmware
      // Note: This list may need to be extended for other firmware variants
      const bufferedCommands = ["G0", "G1", "G2", "G3", "G90", "G91", "M82", "M83"];
      ...
      if (bufferedCommands.includes(cmdType)) {
        ...
        // Debug logging removed - buffer size tracking for firmware compatibility
