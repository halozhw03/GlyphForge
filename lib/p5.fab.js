...
p5.prototype.createFab = function () {
  // 将当前 p5 实例传给 Fab，便于在无 WEBGL 时安全降级
...
class Fab {
  constructor(config = defaultPrinterSettings, p5Instance = null) {
    ...
    // Camera for viz（在无 WEBGL 时提供安全回退）
    const createSafeCamera = () => {
      return {
        // 与 p5.Camera 使用到的接口保持一致
        eyeX: 0,
        eyeY: 0,
        eyeZ: 0,
        centerX: 0,
    ...
    let cameraObj = null;
    // 优先使用实例方法（p5 实例存在且启用了 WEBGL 时有效）
